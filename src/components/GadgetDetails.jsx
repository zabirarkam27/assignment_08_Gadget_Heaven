...
const GadgetDetails = () => {
  ...
  useEffect(() => {
    ...
    const isExistInWishlist = wishlists.find((item) => item.id == itemData.id);

    const isExistInCart = carts.find((item) => item.id == itemData.id);

    setIsWishlist(isExistInWishlist);
    setIsCart(isExistInCart);
  }, [data, id]);

  const handleWishlist = () => {
    if (isWishlist) {
      removeWishlist(gadget.id);
    } else {
      addWishlist(gadget);
    }
          ...
          <div className="card-body">
            ...
            <div className="card-actions justify-start">
              ...
              <button
                ...
                disabled={isCart}
                className={`btn rounded-full  text-white  flex item-center gap-2 group ${
                  isCart
                    ? "bg-gray-500 cursor-not-allowed"
                    : "bg-[#9538E2] hover:bg-white hover:border-[#9538E2] hover:text-[#9538E2] transition-colors duration-300"
                } `}
              >
                {isCart ? (
                  <>
